// File: examples/runtime/03_echo_server.c
// Purpose: nonblocking TCP echo server using runtime API
...
static void handle_client(void *opaque) {
    ...
    minimk_runtime_socket_t client_sock = (minimk_runtime_socket_t)opaque;

    char buffer[4096];
    for (;;) {
        // Read some bytes from the client
        size_t nread = 0;
        minimk_error_t rv = minimk_runtime_socket_recv(client_sock, buffer, sizeof(buffer), &nread);
        ...
        // Echo back the data
        rv = minimk_runtime_socket_sendall(client_sock, buffer, nread);
    ...
    minimk_runtime_socket_destroy(&client_sock);
}
...
static void accept_loop(void *opaque) {
    ...
    for (;;) {
        ...
        minimk_runtime_socket_t client_sock = MINIMK_RUNTIME_INVALID_HANDLE;
        minimk_error_t rv = minimk_runtime_socket_accept(&client_sock, ssock);
...
int main(void) {
    ...
    rv = minimk_runtime_socket_bind(server_sock, "127.0.0.1", "9774");
    if (rv != 0) {
        fprintf(stderr, "Socket bind failed: %s\n", minimk_errno_name(rv));
        minimk_runtime_socket_destroy(&server_sock);
        exit(1);
    }

    // Start listening
    rv = minimk_runtime_socket_listen(server_sock, 128);
    if (rv != 0) {
