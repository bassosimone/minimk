// File: libminimk/socket/create.hpp
// Purpose: create implementation
// SPDX-License-Identifier: GPL-3.0-or-later
#ifndef LIBMINIMK_SOCKET_CREATE_HPP
#define LIBMINIMK_SOCKET_CREATE_HPP
...
MINIMK_ALWAYS_INLINE minimk_error_t minimk_socket_create_impl(minimk_socket_t *sock, int domain, int type,
    ...
    *sock = MINIMK_SOCKET_INVALID;
    ...
    minimk_error_t rv = M_socket(&sockfd, domain, type, protocol);
    ...
    rv = M_setnonblock(sockfd);
    MINIMK_TRACE_SOCKET("setnonblock result=%s\n", minimk_errno_name(rv));

    if (rv != 0) {
        M_closesocket(&sockfd);
        return rv;
    }
...
#endif // LIBMINIMK_SOCKET_CREATE_HPP
